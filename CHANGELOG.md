...
## `0.10.4-horse.14.5` - April 1st, 2026

### Changed

- Bumped version for release (includes all changes from 14.3 and 14.4)
...
- Normalized Windows path separators in bundler bundle keys, fixing alias paths like `/Packages\Commandline.luau`
- Fixed nextest CI install by adding `--locked` flag to `cargo binstall`

### Changed

- Extracted `normalize_bundle_path()` as a shared function with 16 unit tests
...
- Added direct `require(script.Parent.Module)` support - you can now pass a `ScriptReference` directly to `require()` without calling `:RequirePath()`
- Added support for requiring by absolute path (e.g., `require("/home/user/project/module")`)

## `0.10.4-horse.0.4` - January 13th, 2026

### Added

- Added `ScriptReference:RequirePath()` method that returns a require-compatible relative path string
  - Usage: `require(script.Parent.Module:RequirePath())`
  - Automatically calculates relative path from current script to target
- Added support for `default.project.json` (Rojo project files) path resolution
  - Navigation like `script.Parent.Shared` now resolves through project tree definitions
  - Supports `$path` entries in project.json to map virtual names to filesystem paths
...
- Added a `--no-update-luaurc` flag to the `setup` command
